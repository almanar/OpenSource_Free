...
void CallBack_GprsAPNGet(u8 profileid, bool result, s32 error_code, u8 *apn,
		u8 *userId, u8 *password);
void CallBack_getipbyname(u8 contexid, bool result, s32 error, u8 num_entry,
		u8 *entry_address);
...
{ CallBack_network_actived, CallBack_network_deactived, CallBack_socket_connect,
...
ascii hexbuff[6], hex_buff[6], hex_input[2], hex_input_status[2];
ascii SERVER_ADD[15];
float fresult;
char finalp[79]; //,date[9],time[11];
bool input_status, suppy_removed, suppy_connected, intr_check, ac_sense_on,
		ac_sense_off, panic_external, mobiliser, key_talk, panic_status, fuel,
		ignition, output2, key_pickup;
char glo_RX_buff[80];
u8 RXbuf_end_ptr1 = 0;
u8 send;
ascii time[10];
...
u16 port = 2020; //7008
//u16 port = 2023;    //7008
char port1_buffer[] = "2020";					//Server Port No.
char port2_buffer[] = "2020";
char address1_buffer[] = "115.112.176.089";		//Server Address
...
s8 socketonly[2] =
{ 0xFF, 0xFF }; // tcp socket
s8 udponly[2] =
{ 0xFF, 0xFF }; // udp socket
...
QlTimer tm,timer1min,statustimer,fota_timer,filetimer,versiontimer,IpSettingTimer,PulseTimer,AdcTimer;
...
/******************************ODOMETER FUNCTION***********************************************/
/*void distance_calc_TimerHandler ( u8 ID )
 {
 static s8 z;
 ...
 check_dis=0;
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[13], FALSE );//power on pulse
 if(speed_flag==TRUE)
 {
 result_ref=0;
 result_ref1=0;
...
/*if(fix==TRUE)			
 ...
 {
 ntrue=FALSE;
 //adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[0], TRUE );
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[1], TRUE );


 }
 else
 {
 //adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[0], FALSE );
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[1], FALSE );
 ntrue=TRUE;
...
/*
 ...
 {
 ///Output testimg----------------------------------------
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[5], TRUE );//O/P 1
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[6], TRUE );//O/P 2
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[3], TRUE );//O/p 3
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[4], TRUE );//O/P 4
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[7], TRUE );//O/P 5
 ...
 {
 ///Output testimg----------------------------------------
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[5], FALSE );//O/P 1
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[6], FALSE );//O/P 2
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[3], FALSE );//O/p 3
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[4], FALSE );//O/P 4
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[7], FALSE );//O/P 5
 op_cnt=0;
...
/*if(ignition==TRUE)
 {
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[3], TRUE );//O/P 2
 }
 else
 {
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[3], FALSE );//O/P 2
 }*/
/*if(fuel==TRUE)
 {
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[4], TRUE );//O/P 4
...
/*if(ignition==TRUE)
 {
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[6], TRUE );//O/P 3
 }
 else
 {
 adl_ioWriteSingle ( myGpioOut_Handle, &Gpio_Out_Config[6], FALSE );//O/P 3
 }*/
...
/*if(beep_buz== TRUE)
 ...
 if(z<=10)
 {
 adl_atCmdCreate("at+wtone=1,2,25,-1450,2",FALSE, (adl_atRspHandler_t) buzzer_Response_Handler,"*",NULL );
 z++;
 }
 else
 {
 adl_atCmdCreate("at+wtone=1,2,33,-1450,1",FALSE, (adl_atRspHandler_t) buzzer_Response_Handler,"*",NULL );//high sound
 }
 }
 else
 {
...
u8 Uart1_setting_received(void)
{
	...
	if (Uart1_Setting_Buffer[0] == '#' && Uart1_Setting_Buffer[1] == '*'
			&& Uart1_Setting_Buffer[setting_length_counter - 3] == '*'
			&& Uart1_Setting_Buffer[setting_length_counter - 2] == '#'
			&& Uart1_Setting_Buffer[setting_length_counter - 1] == 0x03)
	{
		...
		case 'y'://Employee panic
			if (Dummy_Length == 6)
			{
				...
				Ql_SendToUart(ql_uart_port1,
						(u8*) "Employee Panic Ack Received\r\n",
						Ql_strlen("Employee Panic Ack Received\r\n"));
			}
			else
			{
				// We should not come here.
				Ql_SendToUart(ql_uart_port1,
						(u8*) "You Have Entered Wrong Setting ,Please Enter Write\r\n",
		...
		case 'S':
		case 's':
			if (Dummy_Length == 30)		//String Length for Server IP, Port No and Timer
			{
				if (Uart1_Setting_Buffer[6] == '.'
					&& Uart1_Setting_Buffer[10] == '.'
					&& Uart1_Setting_Buffer[14] == '.')
				{
					for (i = 0; i < 15; i++)
				...
				if (Uart1_Setting_Buffer[18] == 'P'
					|| Uart1_Setting_Buffer[18] == 'p')
				{
					for (i = 0; i < 4; i++)
				...
				if (Uart1_Setting_Buffer[23] == 'M'
						|| Uart1_Setting_Buffer[23] == 'm')
				{
					for (i = 0; i < 3; i++)
...
//        OUT_DEBUG(textBuf,"Ql_GprsNetworkDeactive(contxtid=%d)=%d\r\n",1,ret); 
			...
			else
			{
				Ql_SendToUart(ql_uart_port1,
						(u8*) "You Have Entered Wrong Setting ,Please Enter Write\r\n",
						Ql_strlen(
								"You Have Entered Wrong Setting ,Please Enter Write\r\n"));
				return 0;
			}
			break;
		...
		case 'c':
			if (Dummy_Length == 7)
			{
				...
				if (Uart1_Setting_Buffer[3] == '1')
				{
					...
					Ql_SendToUart(ql_uart_port1, (u8*) buffer,
							Ql_strlen(buffer));
				}
		...
		case 'i':
			if (Dummy_Length == 7)
			{
				if (Uart1_Setting_Buffer[3] == '1')
				{
					iret = Ql_pinWrite(QL_PINNAME_M30_PCM_SYNC,
							QL_PINLEVEL_HIGH);
					Ql_sprintf(buffer, "\r\nWriteHigh(%d),pin=%d\r\n", iret,
		...
		case 'p':
			if (Dummy_Length == 7)
			{
				if (Uart1_Setting_Buffer[3] == '1')
				{
					...
					iret = Ql_pinControl(QL_PINNAME_LIGHT_MOS,
							QL_PINCONTROL_START);
					Ql_sprintf(buffer, "Ql_pinControl(pin=%d, ctrl=%d)=%d\r\n",
...
void save_new_Apn_settings(void)
{
...

	{
		...
		Ql_SendToUart(ql_uart_port1, (u8*) "New_APN_Setting=",
				Ql_strlen("New_APN_Setting="));
...
void save_new_Update_timer_settings(void)
{
...

	{
		...
		// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		Update_Timer_Variable = ((((strBuf[0] - 0x30) * 100)
				+ ((strBuf[1] - 0x30) * 10) + ((strBuf[2] - 0x30) * 1)) / 10);
		Ql_SendToUart(ql_uart_port1, (u8*) "New_Timer_Update_Rate=",
				Ql_strlen("New_Timer_Update_Rate="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
	}
...
void save_new_OwnNumber_settings(void)
{
...

	{
		...
		// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) "New_OWN_Number=",
				Ql_strlen("New_OWN_Number="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
...
void save_new_ip_settings(void)
{
...

	{
		...
		port = (((strBuf[0] - 0x30) * 1000) + ((strBuf[1] - 0x30) * 100)
				+ ((strBuf[2] - 0x30) * 10) + ((strBuf[3] - 0x30) * 1));
		// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) "New_Port_Number=",
				Ql_strlen("New_Port_Number="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
		...
		address[0] = (((strBuf[0] - 0x30) * 100) + ((strBuf[1] - 0x30) * 10)
				+ ((strBuf[2] - 0x30) * 1));
		address[1] = (((strBuf[4] - 0x30) * 100) + ((strBuf[5] - 0x30) * 10)
				+ ((strBuf[6] - 0x30) * 1));
		address[2] = (((strBuf[8] - 0x30) * 100) + ((strBuf[9] - 0x30) * 10)
				+ ((strBuf[10] - 0x30) * 1));
		address[3] = (((strBuf[12] - 0x30) * 100) + ((strBuf[13] - 0x30) * 10)
				+ ((strBuf[14] - 0x30) * 1));

		Ql_SendToUart(ql_uart_port1, (u8*) "New_IP_Address=",
				Ql_strlen("New_IP_Address="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
...
void GPIOPin_Config(void)
{
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_KBR4, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_DCD, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_KBC3, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_RI, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_IN, QL_PINLEVEL_HIGH);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_DISP_RST, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_IN, QL_PINLEVEL_HIGH);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_DISP_CS, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_HIGH);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_DSR, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_IN, QL_PINLEVEL_HIGH);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_DISP_DC, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_IN, QL_PINLEVEL_HIGH);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_GPIO0, QL_PINMODE_1, QL_PINPULLENABLE_DISABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_GPIO1, QL_PINMODE_1, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_KBC4, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));

	//TODO: Check if it is necessary PANIC already Low
	iret = Ql_pinWrite(QL_PINNAME_KBC4, QL_PINLEVEL_LOW);
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_DISP_CLK, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_HIGH);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer,"\r\nSubscribe(%d),pin=%d,mod=%d,pul=%d,dir=%d,lev=%d\r\n", iret,
						QL_PINNAME_M10_DISP_DATA, QL_PINMODE_2, QL_PINPULLENABLE_ENABLE,
						QL_PINDIRECTION_OUT, QL_PINLEVEL_LOW);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
	...
	Ql_sprintf(buffer, "\r\nSubscribe(%d),pin=%d,mod=%d\r\n", iret,
							QL_PINNAME_BUZZER, QL_PINMODE_3);
	Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
...
bool uart1_datahandler(u16 datlength, char * data)
{
	...
	ascii sersettings[150], timerval[6], bufff0 = '0', smsval[6], port[4],
			t_val[3], showcard[150], fset[2], buff[6];
...
void Socket_DataSend_Function(void)
{
	...
	do
	{
		ret = Ql_SocketSend(socketonly[1], Socket_Buffer_API,
...
void FileSyatem_ReadFunction(void)
{
	...
	if (1)
	{
		...
		// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) "ReadPointer=",
				Ql_strlen("ReadPointer="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
		...
		if (Read_Position > 0 && Read_Position <= 2)
			Read_nextposition = (((textBuf[0] - 0x30) * 10)
					+ (textBuf[1] - 0x30));
		else if (Read_Position > 2 && Read_Position <= 3)
			Read_nextposition = (((textBuf[0] - 0x30) * 100)
					+ ((textBuf[1] - 0x30) * 10) + (textBuf[2] - 0x30));
		else if (Read_Position > 3 && Read_Position <= 4)
			Read_nextposition = (((textBuf[0] - 0x30) * 1000)
					+ ((textBuf[1] - 0x30) * 100) + ((textBuf[2] - 0x30) * 10)
					+ (textBuf[3] - 0x30));
		else if (Read_Position > 4 && Read_Position <= 5)
			Read_nextposition = (((textBuf[0] - 0x30) * 10000)
					+ ((textBuf[1] - 0x30) * 1000) + ((textBuf[2] - 0x30) * 100)
					+ ((textBuf[3] - 0x30) * 10) + (textBuf[4] - 0x30));
		else if (Read_Position > 5 && Read_Position <= 6)
			Read_nextposition = (((textBuf[0] - 0x30) * 100000)
					+ ((textBuf[1] - 0x30) * 10000)
					+ ((textBuf[2] - 0x30) * 1000) + ((textBuf[3] - 0x30) * 100)
					+ ((textBuf[4] - 0x30) * 10) + (textBuf[5] - 0x30));
		else if (Read_Position > 6 && Read_Position <= 7)
			Read_nextposition = (((textBuf[0] - 0x30) * 1000000)
					+ ((textBuf[1] - 0x30) * 100000)
					+ ((textBuf[2] - 0x30) * 10000)
					+ ((textBuf[3] - 0x30) * 1000) + ((textBuf[4] - 0x30) * 100)
					+ ((textBuf[5] - 0x30) * 10) + (textBuf[6] - 0x30));

		else if (Read_Position > 7 && Read_Position <= 8)
			Read_nextposition = (((textBuf[0] - 0x30) * 10000000)
					+ ((textBuf[1] - 0x30) * 1000000)
					+ ((textBuf[2] - 0x30) * 100000)
					+ ((textBuf[3] - 0x30) * 10000)
					+ ((textBuf[4] - 0x30) * 1000) + ((textBuf[5] - 0x30) * 100)
					+ ((textBuf[6] - 0x30) * 10) + (textBuf[7] - 0x30));

		else if (Read_Position > 8 && Read_Position <= 9)
			Read_nextposition = (((textBuf[0] - 0x30) * 100000000)
					+ ((textBuf[1] - 0x30) * 10000000)
					+ ((textBuf[2] - 0x30) * 1000000)
					+ ((textBuf[3] - 0x30) * 100000)
					+ ((textBuf[4] - 0x30) * 10000)
					+ ((textBuf[5] - 0x30) * 1000) + ((textBuf[6] - 0x30) * 100)
					+ ((textBuf[7] - 0x30) * 10) + (textBuf[8] - 0x30));

		else if (Read_Position > 9 && Read_Position <= 10)
			Read_nextposition = (((textBuf[0] - 0x30) * 1000000000)
					+ ((textBuf[1] - 0x30) * 100000000)
					+ ((textBuf[2] - 0x30) * 10000000)
					+ ((textBuf[3] - 0x30) * 1000000)
					+ ((textBuf[4] - 0x30) * 100000)
					+ ((textBuf[5] - 0x30) * 10000)
					+ ((textBuf[6] - 0x30) * 1000) + ((textBuf[7] - 0x30) * 100)
					+ ((textBuf[8] - 0x30) * 10) + (textBuf[9] - 0x30));
	...
	if (GPRS_CONNECTION == TRUE)
	{
		...
		if (1)
		{
			...
			Ql_SendToUart(ql_uart_port1, (u8*) "NewReadPosition=",
					Ql_strlen("NewReadPosition="));
		...
		if (pt)
		{
			...
			// Ql_sprintf(textBuf,"Ql_FileGetFilePosition()=%d: Position=%d\r\n",ret, Position);
			Ql_FileSeek(filehandle1, 10, QL_FS_FILE_BEGIN);
			ret1 = Ql_FileWrite(filehandle1, (u8*) pt, Ql_strlen(textBuf),
					&writeedlen);
			Ql_sprintf(textBuf, "Ql_FileWrite()=%d: writeedlen=%d\r\n", ret1,
					writeedlen);
			Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
			...
			// Ql_sprintf(textBuf,"Ql_FileGetFilePosition()=%d: Position=%d\r\n",ret, Position);
			Ql_SendToUart(ql_uart_port1, (u8*) "Read_Position=",
					Ql_strlen("Read_Position="));
...
void iosend_function(void)
{
	...
	if (1)
	{
		...
		ret = Ql_FileRead(filehandle1, strBuf, 10, &readedlen);
		// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) "Writepointer=",
				Ql_strlen("Writepointer="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
		// Ql_SendToUart(ql_uart_port1,strBuf,Ql_strlen( strBuf));
		...
		if (Position1 > 0 && Position1 <= 2)
			...
			nextposition = (((textBuf[0] - 0x30) * 100)
					+ ((textBuf[1] - 0x30) * 10) + (textBuf[2] - 0x30));
		else if (Position1 > 3 && Position1 <= 4)
			nextposition = (((textBuf[0] - 0x30) * 1000)
					+ ((textBuf[1] - 0x30) * 100) + ((textBuf[2] - 0x30) * 10)
					+ (textBuf[3] - 0x30));
		else if (Position1 > 4 && Position1 <= 5)
			nextposition = (((textBuf[0] - 0x30) * 10000)
					+ ((textBuf[1] - 0x30) * 1000) + ((textBuf[2] - 0x30) * 100)
					+ ((textBuf[3] - 0x30) * 10) + (textBuf[4] - 0x30));
		else if (Position1 > 5 && Position1 <= 6)
			nextposition = (((textBuf[0] - 0x30) * 100000)
					+ ((textBuf[1] - 0x30) * 10000)
					+ ((textBuf[2] - 0x30) * 1000) + ((textBuf[3] - 0x30) * 100)
					+ ((textBuf[4] - 0x30) * 10) + (textBuf[5] - 0x30));
		else if (Position1 > 6 && Position1 <= 7)
			nextposition = (((textBuf[0] - 0x30) * 1000000)
					+ ((textBuf[1] - 0x30) * 100000)
					+ ((textBuf[2] - 0x30) * 10000)
					+ ((textBuf[3] - 0x30) * 1000) + ((textBuf[4] - 0x30) * 100)
					+ ((textBuf[5] - 0x30) * 10) + (textBuf[6] - 0x30));
			...
			nextposition = (((textBuf[0] - 0x30) * 10000000)
					+ ((textBuf[1] - 0x30) * 1000000)
					+ ((textBuf[2] - 0x30) * 100000)
					+ ((textBuf[3] - 0x30) * 10000)
					+ ((textBuf[4] - 0x30) * 1000) + ((textBuf[5] - 0x30) * 100)
			...
			nextposition = (((textBuf[0] - 0x30) * 100000000)
					+ ((textBuf[1] - 0x30) * 10000000)
					+ ((textBuf[2] - 0x30) * 1000000)
					+ ((textBuf[3] - 0x30) * 100000)
					+ ((textBuf[4] - 0x30) * 10000)
			...
			nextposition = (((textBuf[0] - 0x30) * 1000000000)
					+ ((textBuf[1] - 0x30) * 100000000)
					+ ((textBuf[2] - 0x30) * 10000000)
					+ ((textBuf[3] - 0x30) * 1000000)
					+ ((textBuf[4] - 0x30) * 100000)
	...
	if (1)
	{
		...
		// Ql_sprintf(textBuf,"Ql_FileGetFilePosition()=%d: Position=%d\r\n",ret, Position);
		Ql_SendToUart(ql_uart_port1, (u8*) "NewWritePosition=",
				Ql_strlen("NewWritePosition="));
	...
	if (pt)
	{
		...
		ret = Ql_FileGetFilePosition(filehandle1, &Position1);
		// Ql_sprintf(textBuf,"Ql_FileGetFilePosition()=%d: Position=%d\r\n",ret, Position);
		Ql_SendToUart(ql_uart_port1, (u8*) "NewWritePointerCount=",
				Ql_strlen("NewWritePointerCount="));
		Ql_sprintf(textBuf, "%d", Position1);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		;
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
...
{
	int i = 0, k = 0, x, z = 0, a = 0, b = 0, c = 0, d = 0, e = 0, f = 0,
			jj = 0, angle1, angle2;
	...
	//Ql_SendToUart(ql_uart_port1,&Datalen,1);
	{
		...
		if ((send == TRUE))
		{
			...
			/*if(d<=4)
			 ...
			 //adl_fcmSendData(uart1_handler,speed2,3);
			 ...
			 speed_flag=TRUE;
			 speed_flag_cnt=0;
			 //ret3=adl_flhWrite ( fhandle, ID[1984],1,&buff1);
			 if(ret3==OK)
			 {
			...
			//Ql_SendToUart(ql_uart_port1,&status,1);
...
void ql_entry()
{
	...
	char ct;
	u8 array[] =
			"!*0109850421299,000000#*35A125460457733134900000022104802011100FF9999036664F1*#";
	u16 len1;
	u32 temp_readlen = 0;
	...
	AdcTimer.timeoutPeriod = Ql_SecondToTicks(60);
	...
	//Reset_GPS(1);
//	ret = Ql_FileOpenEx((u8*)"xyz.txt" ,QL_FS_READ_ONLY);		//Test ---Sandip
//	TODO:Replace FileOpen() function with FileopenEx()
	...
	if (ret != QL_RET_OK)
	{
		...
		if (p)
		{
			...
			ret = Ql_FileRead(filehandle2, strBuf, 10, &readedlen);
			// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
			Ql_SendToUart(ql_uart_port1, (u8*) "OWN_Number=",
					Ql_strlen("OWN_Number="));
			Ql_sprintf(textBuf, "%s", strBuf);
			Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
			Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
			...
			// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
			Ql_SendToUart(ql_uart_port1, (u8*) "APN_Setting=",
					Ql_strlen("APN_Setting="));
			Ql_sprintf(textBuf, "%s", strBuf);
			Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
			Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
	...
	else
	{
		...
		port = (((strBuf[0] - 0x30) * 1000) + ((strBuf[1] - 0x30) * 100)
				+ ((strBuf[2] - 0x30) * 10) + ((strBuf[3] - 0x30) * 1));
		// Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) "Port_Number=",
				Ql_strlen("Port_Number="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
		...
		Ql_SendToUart(ql_uart_port1, (u8*) "IP_Address=",
				Ql_strlen("IP_Address="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
		...
		Update_Timer_Variable = ((((strBuf[0] - 0x30) * 100)
				+ ((strBuf[1] - 0x30) * 10) + ((strBuf[2] - 0x30) * 1)) / 10);
		Ql_SendToUart(ql_uart_port1, (u8*) "Timer_Update_Rate=",
				Ql_strlen("Timer_Update_Rate="));
		Ql_sprintf(textBuf, "%s", strBuf);
		Ql_SendToUart(ql_uart_port1, (u8*) textBuf, Ql_strlen(textBuf));
		Ql_SendToUart(ql_uart_port1, (u8*) "\r\n", Ql_strlen("\r\n"));
	...
	while (keepGoing)		//Main While loop start here.
	{
		...
		case EVENT_INTR:
		{
			...
			Ql_sprintf(buffer, "\r\nEVENT_INTR=pinName(%d),pinState=%d\r\n",
					flSignalBuffer.eventData.intr_evt.pinName,
					flSignalBuffer.eventData.intr_evt.pinState);
			Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
			break;
		}
		...
		case EVENT_KEY:
		{
			...
			Ql_sprintf(buffer, "\r\nEVENT_KEY=key_val(%x),isPressed=%d\r\n",
								flSignalBuffer.eventData.key_evt.key_val,
								flSignalBuffer.eventData.key_evt.isPressed);
			Ql_SendToUart(ql_uart_port1, (u8*) buffer, Ql_strlen(buffer));
		...
		case EVENT_MODEMDATA:
		{
			...
			if (flSignalBuffer.eventData.modemdata_evt.type == DATA_AT)
			{
				...
				if ((Ql_strstr(
						(char*) flSignalBuffer.eventData.modemdata_evt.data,
						"Call Ready\r\n") != NULL))
				{
					// idxCmd = 1;
					// SendAtCmd();
						...
						"+CMTI:") != NULL))
				{
					...
					while (*pData != '\0')
					{
						...
						if (*pData == ',')
						{
							pData++;
							Message_No = *pData;
							Ql_SendToUart(ql_uart_port1, (u8*) "Message_No:",
									Ql_strlen("Message_No:"));
				...
				if (MESSAGE_RECEIVED_FLAG == TRUE)
				{
...

					{
						...
						if (Message_Buffer[msg_length] == '#'
								&& Message_Buffer[msg_length + 1] == '*')
						{
							setting_length_counter = 0;
							while (Message_Buffer[msg_length] != '\0')
							{

								Uart1_Setting_Buffer[setting_length_counter++] =
										Message_Buffer[msg_length];
								msg_length++;
								if (Message_Buffer[msg_length - 2] == '*'
										&& Message_Buffer[msg_length - 1]
												== '#')
								{
							...
							{
								Ql_SendToUart(ql_uart_port1,
										(u8*) Uart1_Setting_Buffer,
										setting_length_counter);
							}
							else
							{
								Ql_SendToUart(ql_uart_port1,
										(u8*) "You Have Sent Wrong Message Setting.Please Send Correct.\r\n",
										Ql_strlen(
												"You Have Sent Wrong Message Setting.Please Send Correct.\r\n"));
							}
						}
						...
						//at_response[at_var]=0;
							...
							(char*) flSignalBuffer.eventData.modemdata_evt.data,
							at_Message_response) != NULL))

					{
						u8 temp_var = 0;
						Ql_SendToUart(ql_uart_port1,
								(u8*) "SENDING MESSAGE COMMAND\r\n",
								Ql_strlen("SENDING MESSAGE COMMAND\r\n"));
				...
				if (FTP_START_FLAG == TRUE)
				{
							...
							"+QFTPGET:") != NULL))
					{
						...
						//SendAtCmd();
						Ql_SendToUart(ql_uart_port1,
								(u8*) "File Downloaded Succesfully\r\n",
				...
				else if (flSignalBuffer.eventData.uartdata_evt.port == 3)
				{
					...
					while (*pData != '\0')
					{
						...
						if (GPSString[c] == 0x0a)
						{
							...
							{
								Ql_SendToUart(ql_uart_port1, (u8*) GPSString,
										Ql_strlen(GPSString));
							}
		...
		case EVENT_TIMER:
			...
			if (tm.timerId == flSignalBuffer.eventData.timer_evt.timer_id)
			{
				...
				Ql_StopTimer(&tm);
				ret = Ql_GprsAPNSet(5, (u8*) Apn_Setting_Buffer, (u8*) "",
						(u8*) "", CallBack_GprsAPNSet);
				//  ret = Ql_GprsAPNSet(5, (u8*)"internet", (u8*)"", (u8*)"", CallBack_GprsAPNSet);
				OUT_DEBUG((char*)textBuf,
						(char*)"Ql_GprsAPNSet(%d)=%d\r\n", 5, ret);

			}

			else if (filetimer.timerId
			...
			else if (IpSettingTimer.timerId
					== flSignalBuffer.eventData.timer_evt.timer_id)
			{
					...
					== flSignalBuffer.eventData.timer_evt.timer_id)
			{
				...
				if (QL_RET_OK == ret2)
				{
					...
					while (filesize > 0)
					{
						...
						if (filesize <= 512)
						{
							Ql_FileRead(fd_file, (u8*) file_buffer, filesize,
									&temp_readlen);
							Ql_Fota_Core_Write_Data(filesize,
									(s8*) file_buffer);
							filesize = 0;
						}
						else
						{
							Ql_FileRead(fd_file, (u8*) file_buffer, 512,
									&temp_readlen);
...
{
	OUT_DEBUG((char *)textBuf,
			(char *)"CallBack_GprsAPNSet(result=%d,error_code=%d)\r\n", result, error_code);
	if (result == TRUE)
	{
		//p2="APNGet=5";
...
{
	OUT_DEBUG((char *)textBuf,
			(char *)"CallBack_GprsAPNGet(profileid=%d,result=%d,error_code=%d,apn=%s,userId=%s,password=%s)\r\n", profileid, result, error_code, apn, userId, password);
...
void CallBack_network_actived(u8 contexid)
{
	...
	case QL_SOC_SUCCESS:
		...
		OUT_DEBUG((char *)textBuf,
				(char *)"Ql_SocketCreate(contxtid=%d)=%d\r\n", 1, ret);
		if (ret < 0)
		{
		...
		OUT_DEBUG((char *)textBuf,
				(char *)"Ql_SocketConnect(ip=%d,%d,%d,%d,port=%d)=%d\r\n", address[0], address[1], address[2], address[3], port, ret);
		if (ret == 0)
		{
			OUT_DEBUG((char *)textBuf,
					(char *)"Ql_SocketConnect connect successed\r\n");
		}
		else if (ret == -2)
		{
			OUT_DEBUG((char *)textBuf,
					(char *)"please wait CallBack_socket_connect\r\n");
		}
		else
		{
			OUT_DEBUG((char *)textBuf,
					(char *)"Ql_SocketConnect run error\r\n");
		}
...
{
	OUT_DEBUG((char *)textBuf,
			(char *)"CallBack_network_deactived(contexid=%d,error_cause=%d, error=%d)\r\n", contexid, error_cause, error);
	if (socketonly[0] >= 0)
	{
...
void CallBack_socket_connect(u8 contexid, s8 sock, bool result, s32 error)
{
	s32 ret;
	OUT_DEBUG((char *)textBuf,
			(char *)"CallBack_socket_connect(contexid=%d,sock=%d,result=%d,error=%d)\r\n", contexid, sock, result, error);
...
void CallBack_socket_close(u8 contexid, s8 sock, bool result, s32 error)
{
	...
	OUT_DEBUG((char *)textBuf,
			(char *)"CallBack_socket_close(contexid=%d,sock=%d,result=%d,error=%d)\r\n", contexid, sock, result, error);
	Ql_SocketClose(sock);
...
void CallBack_socket_read(u8 contexid, s8 sock, bool result, s32 error)
{
	...
	OUT_DEBUG((char *)textBuf,
			(char *)"%s:CallBack_socket_read(contexid=%d,sock=%d,result=%d,error=%d)\r\n", istcp?"TCP":"UDP", contexid, sock, result, error);
	...
	if (istcp) // tcp recv
	{
		do
		{
			...
			{
				OUT_DEBUG((char *)textBuf,
						(char *)"TCP Ql_SocketRecv()=%d, error exit\r\n", ret);
			...
			else if (ret < 1024)
			{
				...
				if (data_r[0] == '#' && data_r[1] == '*')
				{
					...
					case 'D':
						...
						ret = Ql_GprsNetworkDeactive(1);
						OUT_DEBUG((char *)textBuf,
								(char *)"Ql_GprsNetworkDeactive(contxtid=%d)=%d\r\n", 1, ret);
						FTP_START_FLAG = TRUE;
						idxCmd = 1;
						Ql_SendToUart(ql_uart_port1,
								(u8*) "DOTA COMMAND RECEIVED\r\n",
								Ql_strlen("DOTA COMMAND RECEIVED\r\n"));
						SendAtCmd();
						//  data_r[2]=0;
						break;
	...
	else // udp recv
	{
		...
		do
		{
			...
			{
				OUT_DEBUG((char *)textBuf,
						(char *)"UDP Ql_SocketRecvFrom()=%d, error exit\r\n", ret);
				Ql_SocketClose(sock); //you can close this socket
				if (udponly[0] == sock)
				{
					udponly[0] = 0xFF;
...
void CallBack_socket_write(u8 contexid, s8 sock, bool result, s32 error)
{
	...
	s32 i;
	OUT_DEBUG((char *)textBuf,
			(char *)"CallBack_socket_write(contexid=%d,sock=%d,result=%d,error=%d)\r\n", contexid, sock, result, error);
	...
	{
		ret = Ql_SocketSend(sock, data_p + send_dataLen[i],
				dataLen[i] - send_dataLen[i]);
		OUT_DEBUG((char *)textBuf,
				(char *)"Ql_SocketSend(socket=%d,dataLen=%d)=%d\r\n", socketonly[i], dataLen[i], ret);
