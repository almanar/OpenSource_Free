...
#define NO_OF_WAVE_POINT_STORED 1000
#define ADC_UPDATE_RATE 1 
...
char versionstring[]="#*05M30FVERSION001*#";
...
u32 Update_Timer_Variable=60;
ascii dummy_data[]="000000";
//u16 port = 3456;
...
u32 input;
u8 adc_updatearray=0;
u16 adc_avg_value=0;
u16 adc_array[4]={0,0,0,0};
...
void UpdateSpeedinMemory(void)
{
	 ...
	 u32 Temp_Data_Var=0;
	 u32 ret34;
	 Temp_Data_Var=fresult;
	 OpenFeatures_File();
	 dummy_data[0]=(Temp_Data_Var/100000)+0x30;
	 Temp_Data_Var%=100000;
	 dummy_data[1]=(Temp_Data_Var/10000)+0x30;
	 Temp_Data_Var%=10000;
	 dummy_data[2]=(Temp_Data_Var/1000)+0x30;
	 Temp_Data_Var%=1000;
	 dummy_data[3]=(Temp_Data_Var/100)+0x30;
	 Temp_Data_Var%=100;
	 dummy_data[4]=(Temp_Data_Var/10)+0x30;
	 Temp_Data_Var%=10;
	 dummy_data[5]=(Temp_Data_Var/1)+0x30;
	 
	 
	  
     Ql_memset(textBuf,0,63);
    
     ret34 = Ql_FileWrite(filehandle3, (u8*)dummy_data,6, &writeedlen);
     Ql_sprintf(textBuf,"Ql_FileWrite()=%d: writeedlen=%d\r\n",ret34, writeedlen);
     Ql_SendToUart(ql_uart_port1,(u8*)textBuf,Ql_strlen(textBuf));
     ...
     Ql_memset(strBuf,0,63);
     Ql_memset(textBuf,0,63);
     Ql_FileSeek(filehandle3,0, QL_FS_FILE_BEGIN);
     ret34 = Ql_FileRead(filehandle3, strBuf,6,&readedlen);
     // Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
     Ql_SendToUart(ql_uart_port1,(u8*)"SPEED_DATA_WRITTEN=",Ql_strlen("SPEED_DATA_WRITTEN="));
     Ql_sprintf(textBuf,"%s",strBuf);
	 Ql_SendToUart(ql_uart_port1,(u8*)textBuf,Ql_strlen(textBuf));
	 Ql_SendToUart(ql_uart_port1,(u8*)"\r\n",Ql_strlen("\r\n"));
	  
	  Ql_FileClose(filehandle3);
      filehandle3 = -1;
      Ql_memset(textBuf,0,63);
      Ql_sprintf(textBuf,"Ql_FileClose()\r\n");
      Ql_SendToUart(ql_uart_port1,(u8*)textBuf,Ql_strlen(textBuf));
  ...
  //  u8 reset[]="!*0109850421299#*35A2833822277074093022616130425211210001E999903CBBA01*#";
   ...
   // Ql_DebugTrace("The timer starts.\r\n\r\n");
    ...
    p=port1_buffer;
	 ...
	 AdcTimerSend.timeoutPeriod=Ql_SecondToTicks(ADC_UPDATE_RATE*Update_Timer_Variable);
	 Ql_StartTimer(&AdcTimerSend);
	...
	else
	{
	 ...
	 //speed setting
	ret=Ql_FileCheck((u8*)"features.txt");
    
    if(ret!=QL_RET_OK)
    { 
    p1="Ql_FileOpen=cfeatures.txt";
    p = Ql_strstr(p1,"Ql_FileOpen=c");
    if (p)
    {
      // s32 ret;
    ...
    p=dummy_data;
     ...
     ret = Ql_FileWrite(filehandle3, (u8*)p,6, &writeedlen);
     Ql_sprintf(textBuf,"Ql_FileWrite()=%d: writeedlen=%d\r\n",ret, writeedlen);
     Ql_SendToUart(ql_uart_port1,(u8*)textBuf,Ql_strlen(textBuf));
     ...
     Ql_memset(textBuf,0,63);
     Ql_FileSeek(filehandle3,0, QL_FS_FILE_BEGIN);
     ret = Ql_FileRead(filehandle3, strBuf,6,&readedlen);
     // Ql_sprintf(textBuf,"Ql_FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
     Ql_SendToUart(ql_uart_port1,(u8*)"NEW_SPEED_COUNT=",Ql_strlen("NEW_SPEED_COUNT="));
     Ql_sprintf(textBuf,"%s",strBuf);
	 Ql_SendToUart(ql_uart_port1,(u8*)textBuf,Ql_strlen(textBuf));
	 Ql_SendToUart(ql_uart_port1,(u8*)"\r\n",Ql_strlen("\r\n"));
	...
	else
	{
     ...
     Ql_FileSeek(filehandle3,0, QL_FS_FILE_BEGIN);
     ret = Ql_FileRead(filehandle3, strBuf,6,&readedlen);
	 fresult=ASCIItoINT((ascii *)strBuf,6);
	 Ql_sprintf(buffer, "\r\nold_fresult=%d\r\n",fresult);
     Ql_SendToUart(ql_uart_port1,(u8*)buffer,Ql_strlen(buffer));
	 Ql_SendToUart(ql_uart_port1,(u8*)"\r\n",Ql_strlen("\r\n"));
     Ql_SendToUart(ql_uart_port1,(u8*)"OLD_SPEED_COUNT=",Ql_strlen("OLD_SPEED_COUNT="));
     Ql_sprintf(textBuf,"%s",strBuf);
	 Ql_SendToUart(ql_uart_port1,(u8*)textBuf,Ql_strlen(textBuf));
	 Ql_SendToUart(ql_uart_port1,(u8*)"\r\n",Ql_strlen("\r\n"));
   ...
   SendGPS_Setting();
    ...
    while(keepGoing)
    {
        ...
        switch(flSignalBuffer.eventType)
        {
                   ...
                   if(GPSString[c]==0x0a)
                    {
	                   ...
	                   GPSString[c]='\0';
					   GpsCheckSum=0;
					   for(k1=1;GPSString[k1]!='*';k1++)
					   {
					   	GpsCheckSum^=GPSString[k1];
					   }
					   k1++;
					   GpsReceivedCheckSum=0;
					   if(GPSString[k1]>='0' && GPSString[k1]<='9')
					   {
						 GpsReceivedCheckSum=GPSString[k1]-0x30;
					   }
					   else
					   {
						GpsReceivedCheckSum=GPSString[k1]-0x37;
					   }
					   GpsReceivedCheckSum<<=4;
					   k1++;
					   if(GPSString[k1]>='0' && GPSString[k1]<='9')
					   {
						 GpsReceivedCheckSum|=GPSString[k1]-0x30;
					   ...
					   if(GpsCheckSum==GpsReceivedCheckSum)
					   {
						...
						{
						Ql_SendToUart(ql_uart_port1,(u8*)"INCORRECT GPS CHECKSUM\r\n",Ql_strlen("INCORRECT GPS CHECKSUM\r\n"));
			...
			else if( AdcTimerSend.timerId  == flSignalBuffer.eventData.timer_evt.timer_id)
            {
                        
             Ql_StopTimer(&AdcTimerSend);
			 AdcTimer.timeoutPeriod=Ql_SecondToTicks(10);
...
void Callback_Read_ADC(QlADCPin adc_pin, u8 status, u16 adc_val)
{
			 ...
			 adc_array[adc_updatearray]=(adc_val-22);
			 adc_updatearray++;
			 adc_avg_value=(adc_array[0]+adc_array[1]+adc_array[2]+adc_array[3])/4;
			 
			 
			 if(adc_updatearray>=4)
			 {
			 adc_value[3]=adc_avg_value/1000;
			 adc_value[3]+=0x30;
			 adc_avg_value%=1000;
			 adc_value[2]=adc_avg_value/100;
			 adc_value[2]+=0x30;
			 adc_avg_value%=100;
			 adc_value[1]=adc_avg_value/10;
			 adc_value[1]+=0x30;
			 adc_avg_value%=10;
			 adc_value[0]=adc_avg_value;
			 adc_value[0]+=0x30;
			 }
